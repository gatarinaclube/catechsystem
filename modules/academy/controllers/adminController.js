...
function portalArticleFromBody(body, files, prefix, index, fileField) {
  ...
  return {
    ...
    imagePositionX: arrayAt(body[`${prefix}ImagePositionX`], index, 50),
    imagePositionY: arrayAt(body[`${prefix}ImagePositionY`], index, 50),
    imageScale: arrayAt(body[`${prefix}ImageScale`], index, 100),
    imageFit: arrayAt(body[`${prefix}ImageFit`], index, "cover"),
    bodyImages: Array.from({ length: 3 }, (_, slot) => ({
      imageUrl: fileUrlAt(files, `${prefix}BodyImage${index}_${slot}`) || arrayAt(body[`${prefix}BodyImageUrl${index}_${slot}`], 0),
      caption: arrayAt(body[`${prefix}BodyImageCaption${index}_${slot}`], 0),
    })),
